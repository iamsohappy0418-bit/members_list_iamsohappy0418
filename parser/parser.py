...
def parse_natural_query(text: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    자연어에서 (필드, 키워드) 추출
    우선순위:
      1) '회원조회' 구문: 숫자=회원번호, 그 외=회원명
      2) '조회/검색/찾아' 구문: 맨 앞 토큰=회원명
      3) 고급 필드: 계보도/소개한분/코드/분류/리더님/회원번호
    """
    if not text:
        return None, None
    s = text.strip()
...
def process_order_date(raw: Optional[str]) -> str:
...
def parse_korean_phone(text: str) -> Optional[str]:
    m = re.search(r"01[016789]-?\d{3,4}-?\d{4}", text or "")
    if not m:
        return None
    digits = re.sub(r"\D", "", m.group())
    return f"{digits[:3]}-{digits[3:7]}-{digits[7:]}"

def parse_member_number(text: str) -> Optional[str]:
...
# --- 회원 수정 파싱 ------------------------------------------------------------
def infer_field_from_value(v: str) -> Optional[str]:
    v = (v or "").strip()
...
# --- 주문 텍스트(규칙 기반) ----------------------------------------------------
def parse_order_text_rule(text: str) -> Dict[str, Any]:
    """
...
def parse_request_line(text: str):
    ...
    s = text.strip()
    m = re.match(
        r"^\s*(\S+)\s*(상담\s*일지|개인\s*일지|활동\s*일지|회원\s*메모|회원\s*주소|상담일지|개인일지|활동일지|회원메모|회원주소)\s*(저장|기록|입력)\s*(.*)$",
        s,
    )
    if m:
...
def match_condition(text: str, keywords: List[str], mode: str) -> bool:
    if not keywords:
        return True
    t = (text or "").lower()
